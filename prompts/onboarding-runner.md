...
2. **Set up the working folder and get the brand's repository from Parker — do not build inside this `parker-brain` clone.** This product brain is the read-only factory teams clone; the brand brain is a **separate, standalone repo for that brand** — and not one you create by hand: Parker provisions it. Confirm with the user where their working folder should live, then maintain this shape inside it:

   - `AGENTS.md` — a short top-level guide any AI agent reads on opening the folder. Write it now (or update it) so a fresh session can orient itself without you: it points at `factory/README.md` for what Parker Brain is and how a brand brain gets built, and at the **`setup_parker_brain` tool description** (Parker MCP) for the repo workflow the agent must follow — fresh credentials come from re-calling the tool, pull before reading, commit and push after every edit, resolve conflicts best-effort.
   - `factory/` — the clone of the public `parker-brain` repo: the prompts, skills, and method. Read-only — `git pull` it for the latest method, never commit brand work into it.
   - `<brand name>-brain/` — the brand's own repo, cloned below. Every brand output is committed here, never back into the factory. The flat standalone layout above is the shape of this repo.

   Then stand the brand repo up through the **`setup_parker_brain` tool** (Parker MCP), using the brand_id locked in step 1 — read that tool's description and follow it; it is the canonical contract for working with this repo. Before calling it, ask whether anyone on the team wants direct GitHub access to the brain: the tool optionally takes a `github_username` to invite as a collaborator — but make clear this is optional, and the user needs no GitHub account at all, because the repo lives under Parker's GitHub organization and you work with it entirely through the tool's credentials. The call is idempotent: it **creates the brand's private repo if none exists, or retrieves the existing one** (`reused_existing: true`) — it never wipes or regenerates anything. It returns an `authenticated_clone_url` with a short-lived (~1 hour) token embedded — a secret: never print it, never commit it — plus `token_expires_at`. Clone that URL into `<brand name>-brain/`; when the token lapses or a git operation hits an auth error, call the tool again for a fresh URL and update the remote.
...
2. Push to the brand repo's `main` on GitHub (`git push -u origin main`). The `origin` remote is the tokenized `authenticated_clone_url` from `setup_parker_brain`, and that token lives about an hour — a build this long has almost certainly outlived it. If the push is rejected with an auth error, call `setup_parker_brain` again, point the remote at the fresh URL (`git remote set-url origin <fresh url>`), and push; re-minting is cheap and expected.
3. Confirm it landed: give them the repo URL (`html_url`), tell them everything is now saved there, and explain in one line what that means — their brain is backed up in the cloud, and they (or a teammate, or a future session) can pick it up any time: a new session re-clones through `setup_parker_brain`, and anyone invited as a collaborator can open the repo on GitHub directly.
