...
2. **Find the brand's folder.** The normal entry is that **Parker Desktop already opened this session inside the brand's synced folder** — the team created the brand's repository in the app, and the app synced the folder down and handed off to Claude. Confirm that's where you are — and that it's the *right brand's* folder: read `~/.parker/workspace.json` (the pointer file the app maintains — its `root` is the parker folder, one subfolder per brand), check the working directory sits under that root, and check identity against the locked brand_id — the folder's `parker_config.json` carries `brand_id` on any brain past Phase 0, and a fresh empty folder is matched by name (ask if unclear). A mismatch means you're in another brand's folder: stop and switch before writing anything. If you're somewhere else entirely — say, a factory clone — look under the root for the brand's subfolder the same way. If the pointer file is missing or no brand folder exists under it, the repo hasn't been created yet (or the app isn't installed): that's Phase 0 step 2's conversation, not a dead end.
...
Then write `parker_config.json` to the repo root with the returned `run_id` (it saves and syncs like any other file). This is the cross-session resume anchor — if it is not written before the session ends, the next session cannot resume and will start a new run.
...
5. **Mount the factory method at `parker-system/` and ship the executables into `.claude/`.** No prompt does this; the runner does. Three moves:
   ...
   **First, the mount.** Add the public factory as a pinned submodule and lock it to the latest release:
   ...
   These are local, credential-free operations against the **public** factory — the one kind of git this repo still sees; Parker Desktop saves and syncs the resulting mount like any other change.
   ...
   **Fourth, the voice layer.** Copy `parker-system/.claude/output-styles/parker.md` into the brand brain's `.claude/output-styles/` (create the directory — output styles, like skills and agents, only register from `.claude/`). This is the system-prompt layer that makes Parker talk like Parker in every session: it replaces the harness's dense terminal default with the plain, warm, tenth-grade register, while `keep-coding-instructions: true` keeps the careful file-work discipline. The routine bundle's stamped `.claude/settings.json` carries the `"outputStyle": "Parker"` switch, so the voice is live the moment the brain is cloned — no setup step for the team. Both halves land or neither works: the switch without the file, or the file without the switch, leaves Parker sounding like a terminal.

   **Fifth, the Codex twin.** The same brain must open cleanly in OpenAI Codex, and that layer is three small artifacts (contract: `parker-system/system/codex-support.md`). Copy `parker-system/templates/brand-routines/codex/` → `.codex/` (the committed Codex project config wiring the same hooks) and `parker-system/templates/brand-routines/AGENTS.md` → `AGENTS.md` at the repo root (Codex's entry point: routes to `CLAUDE.md`, carries the voice). Then create the skills symlink: `ln -s ../.claude/skills .agents/skills` (create `.agents/` first) — Codex discovers skills from `.agents/skills/`, and the symlink means both harnesses read the same files with nothing to keep in sync. No Codex-side setup runs during the build; the per-user trust-and-approve flow happens the first time someone opens the brain in Codex, and `.codex/README.md` walks them through it.
...
1. Confirm the sync is live: the folder sits under the `root` in `~/.parker/workspace.json` and the Parker Desktop app is running. If either looks off — the pointer file is missing, the folder is outside its root — say so plainly, point them at https://app.heyparker.ai/dashboard/parker-desktop, and don't call the build saved until it's syncing. (A self-managed team commits and pushes with their own connection instead.)
