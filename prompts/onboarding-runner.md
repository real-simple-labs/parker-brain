...
2. **Find the brand's folder.** The normal entry is that **Parker Desktop already opened this session inside the brand's synced folder** — the team created the brand's repository in the app, and the app synced the folder down and handed off to Claude. Confirm that's where you are: read `~/.parker/workspace.json` (the pointer file the app maintains — its `root` is the parker folder, one subfolder per brand) and check the working directory sits under that root. If you're somewhere else — say, a factory clone — look under the root for a subfolder matching the brand, and ask the user if it's genuinely unclear. If the pointer file is missing or no brand folder exists under it, the repo hasn't been created yet (or the app isn't installed): that's Phase 0 step 2's conversation, not a dead end.
