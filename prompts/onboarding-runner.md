...
2. **Ask Parker for the repo.** Call `setup_parker_brain` with the brand_id. It's idempotent and never wipes or regenerates anything; `reused_existing: true` means the brand already has a repo. If it does, clone it (a plain `git clone --recurse-submodules` — the machine-level sync helper authenticates, per Phase 0 step 2) and look inside before saying a word about building.
3. **Route by what you find:**
   ...
   - **New or empty** → a real cold start. Proceed to the orientation below; Phase 0 step 2 reuses the repo you just provisioned (the call is idempotent — repeating it is safe and cheap).
...
2. **Get the brand's repository from Parker — do not build inside a `parker-brain` clone.** The brand brain is a **separate, standalone repo for that brand**, and not one you create by hand: Parker provisions it. Stand it up through the **`setup_parker_brain` tool** (Parker MCP), using the brand_id locked in step 1 — read that tool's description and follow it; it is the canonical contract for working with this repo. Before calling it, ask whether anyone on the team wants direct GitHub access to the brain: the tool optionally takes a `github_username` to invite as a collaborator — but make clear this is optional, and the user needs no GitHub account at all, because the repo lives under Parker's GitHub organization and syncing is handled by Parker's machine-level helper. The call is idempotent: it **creates the brand's private repo if none exists, or retrieves the existing one** (`reused_existing: true`) — it never wipes or regenerates anything. The call returns the repo's **plain** clone URL — and that's all it needs to return, because auth is machine-level, not per-repo. Before cloning, make sure this machine has the Parker sync helper (`~/.parker/bin/parker-credential` exists); if it doesn't, run the one-time `npx @heyparker/sync setup`, telling the user in plain words what's happening ("I'll have you sign in to Parker once — a browser window will open; after this, saving and syncing your brain is automatic"). Then clone plainly: `git clone --recurse-submodules https://github.com/parker-brain/<repo>.git <folder>` — no tokens, no credential files, no special flags; from here on the helper authenticates every git operation, minting a short-lived, single-repo credential server-side each time, so nothing ever expires mid-build and nothing secret ever enters a command, a file, or the chat. The full procedure is the `save-brain` skill, at `templates/brand-routines/claude/skills/save-brain/SKILL.md` in this factory and later shipped into the brain as `/save-brain`; follow it for every git operation here. Confirm with the user where on their machine the brain should live, and make the cloned repo the working directory for everything below — the brand repo *is* the workspace; there is no wrapper folder around it. **All git work here is plain `git` — never the `gh` CLI.** The sync helper carries all the auth this repo needs, the user's own GitHub identity is never involved, and `gh` only introduces sign-in prompts and auth failures for credentials nobody has to have. (`gh` reaching for the *user's* login is exactly the failure mode the tool-provisioned repo exists to avoid.)
...
2. Push to the brand repo's `main` on GitHub (`git push -u origin main`). Auth is the machine-level sync helper, which mints a fresh credential per operation — a build of any length never outlives it. If the push still fails with an auth error, walk `/save-brain`'s ladder: clean any legacy per-repo credential wiring, confirm the helper is installed (the one-time `npx @heyparker/sync setup`, user present), retry. If the build ran in a hosted session that assigned a working branch, the save still lands on `main` — push `HEAD:main`; `/save-brain`'s no-branches rule is the explicit permission the harness asks for. Full procedure: the `save-brain` skill (`templates/brand-routines/claude/skills/save-brain/SKILL.md` in the factory, `/save-brain` once shipped into the brain).
