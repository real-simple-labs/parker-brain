...
- `parker-system/` — the rest of the factory methodology, copied in during Phase 0 so the brain is self-contained and can refresh, rebuild, and reason on its own with no dependency on the factory at runtime. The layers under it (three methodology layers, plus a small `fixtures/` folder of example artifacts the prompts reference):
...
5. **Ship the factory method into the brand repo — the skills into `.claude/skills/` and the rest into `parker-system/`.** Copy the whole factory method in so the brain is self-contained and references nothing in the factory at runtime. No prompt does this; the runner does. Two destinations:
   ...
   **First, the skills — into `.claude/skills/`.** This is the only directory Claude Code loads skills from; a skill copied anywhere else (a top-level `skills/`, or under `parker-system/`) is inert and a clone never registers it. Copy the **entire** factory craft-skill set from the factory's own `.claude/skills/` into the brand brain's `.claude/skills/`, *alongside* the routine bundle stamped from `templates/brand-routines/` — every craft skill, not a curated subset: the creative-execution skills (`scriptwriting`, `hooks`, `headlines`, `iterations`, `ad-account-analysis`, `ai-ad-generation`), the open-loops pipeline (`open-loops-advance`, `open-loops-validate`), idea-bank maintenance (`brand-idea-bank-maintenance`), and the living-loop / intake skills (`improve-system`, `self-improvement-intake`, `expert-signal-intake`). The few factory-maintenance skills (`propagate-craft`, `update-parker-skill`) come along too and sit inert in a brand brain, the same way the factory-orchestration prompts do — don't curate them out. **One name collision to respect:** the factory craft set and the routine bundle both define a `dream` skill; the routine `dream` (the scheduled dreaming run, the brain's canonical one) wins, so do not overwrite it with the craft `dream`. After this, the brand `CLAUDE.md` routes every execution-shaped task through `.claude/skills/<skill>/`, and on clone they all load after the workspace-trust prompt. **A straight copy is not enough — the paths need normalizing.** The factory's skills and agents reference the craft layer by factory paths (`global/knowledge/creative-strategy/...`, `system/<runtime-doc>.md`) that do not exist in the brain's layout. After the skills and the review-gate agents below are both in place, run the deterministic rewrite from the factory clone: `python3 scripts/normalize-brain-paths.py <brand-repo-path> nested`. It rewrites every method-doc reference in the brain's `.claude/skills/` and `.claude/agents/` to the `parker-system/` paths that actually resolve there, and it is idempotent. Skipping this leaves every craft skill pointing at docs that do not exist, silently degrading day-one output to whatever the model can glob its way to.

   **Alongside the skills, the creative review gates.** The creative skills' ship gates spawn two independent reviewers, and that machinery is four more files the skills reference by name: copy the factory's `.claude/agents/creative-voice-review.md` and `.claude/agents/context-grounding-review.md` into the brand brain's `.claude/agents/` (create the directory — agents, like skills, only register from `.claude/`), and `scripts/voice-lint.py`, `scripts/grounding-check.py`, `scripts/normalize-brain-paths.py`, plus `scripts/verify-brain.py` into the brain's `scripts/` (create it too — the normalizer and verifier ship so the brain can re-check its own harness any time). The doctrine the voice pair enforces, `ai-writing-tells.md`, arrives with the craft-knowledge copy below; the grounding pair enforces the routing map's test against the brain's own vault. All of it lands or the gates in `scriptwriting`, `headlines`, `hooks`, `ai-ad-generation`, and `iterations` point at nothing. **And the cross-agent contract:** stamp `templates/brand-brain-AGENTS.md` to the brain root as `AGENTS.md`, filling its `{{BRAND_HARD_RULES}}` placeholder with the same hard rules stamped into `CLAUDE.md` (the two are mirrors; note in both that edits go to both). Teams connect other agents (Manus, Codex, Cursor) to their brain; those agents never read `CLAUDE.md` or register `.claude/` machinery, and `AGENTS.md` is the convention they do read — it is the brain's canonical harness-agnostic contract, carrying the routing and gate rules in a form any agent can follow. **And the harness-agnostic scheduler:** copy `templates/brand-routines/github/workflows/parker-routines.yml` to the brain's `.github/workflows/` — the cron alarm clock that runs the six routines through whichever agent CLI the team configures (`PARKER_AGENT_RUNNER` repo variable + an API-key secret; `none` keeps it silent for teams using Claude scheduled agents instead). Tell the user during handoff that arming it is a two-minute Settings task, and that they pick ONE scheduler — Actions or Claude cloud agents, never both.
   ...
   **Then `parker-system/`** — create it and copy in the other three layers:
   ...
   - **Fixtures:** copy `fixtures/` to `parker-system/fixtures/` — the small sanitized example artifacts the prompts reference (currently the creative-tracker example the `sprint-plan` prompt points to). These are illustrations the brain can open, never brand data; the brand's own populated tracker is what the `sprint-plan` step produces each round.

   The prompts ship already normalized to `parker-system/` paths — they reference `parker-system/creative-strategy-context/`, `parker-system/system/`, `parker-system/prompts/`, and `parker-system/fixtures/` rather than factory paths, so a copy is all that's needed and the references resolve as-is. The brand data the build produces (sub-context-docs, personas, audits, strategy, the loop pipeline, running-notes) stays flat at the repo root per the path map above; the factory method lives under `parker-system/`, with the executable `skills/` kept as its own top-level folder beside it.
...
**First, the one deterministic check: run `python3 scripts/verify-brain.py` from the brain root.** It mechanically verifies every link in the harness chain — all 21 skills registered where Claude Code loads them, both reviewer agents, the checkers present *and executing* (it fires voice-lint at a known slop line and expects flags), the hooks and settings valid, the craft docs shipped, zero unresolvable factory paths in `.claude/`, and `.claude/` actually tracked by git. Every line must say PASS; a single FAIL means the build is not done — fix it and re-run until the result line reads PASS, then paste the full output into `BUILD-STATUS.md` as the harness receipt. This script is the check that would have caught every shipping miss found in the wild so far; nothing below substitutes for it.

Then spawn a review subagent whose only job is structural completeness beyond what the script can see. Give it this runner and the brand repo, and have it verify each of these, reporting **pass** or **fail with the specific gap**:
