...
2. **Find the brand's folder.** The normal entry is that **Parker Desktop already opened this session inside the brand's synced folder** — the team created the brand's repository in the app, and the app synced the folder down and handed off to Claude. Confirm that's where you are: the working directory sits under `PARKER_BRAIN_DIR` (the environment variable the app sets), or carries a `parker_config.json`. If you're somewhere else — say, a factory clone — check `PARKER_BRAIN_DIR` for a subfolder matching the brand, and ask the user if it's genuinely unclear. If no synced folder exists anywhere, the repo hasn't been created yet: that's Phase 0 step 2's conversation, not a dead end.
...
1. Confirm the sync is live: the folder sits under `PARKER_BRAIN_DIR` and the Parker Desktop app is running. If either looks off — the app isn't installed, the folder is outside it — say so plainly, point them at https://app.heyparker.ai/dashboard/parker-desktop, and don't call the build saved until it's syncing. (A self-managed team commits and pushes with their own connection instead.)
