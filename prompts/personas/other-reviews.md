...
The methodology for reading customer reviews — the three-way hunt for nuggets, messaging opportunities, and whole-review concepts; the qualifying signals; the exclusion list; bucketing by SKU and trigger; the claims-check and voice-check governors; era tagging; and the common failure modes — lives in `global/knowledge/creative-strategy/customer-review-mining-method.md`, which should be loaded alongside this prompt. This prompt encodes the persona-signal slice for third-party reviews specifically, including the distinction in trust and selection bias between independent surfaces and the brand's own.
