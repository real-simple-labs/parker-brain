...
- **Start from the scaffold.** The brand folder gets its starting files (the method copies, the skills, the empty running notes) from `scripts/scaffold-brain.py`, never by hand. Often Parker's backend already ran it when the brand signed up, and a `.scaffolded` file at the root says so; otherwise the runner's Phase 0 step 3 runs it. A scaffolded brain isn't built yet. The marker comes off in code once the build reports its first finished phase after Phase 0, and closeout removes it if it's still there (`system/brain-scaffold.md`).
