...
This is a concept the rest of the prompts depend on, so understand it completely. The operational core is embedded verbatim in every context-doc prompt as the open-loops core block, synced from `prompts/_open-loops-core-block.md`, so the rubric is in context wherever the prompt runs without depending on a file load. The system architecture sits in `system/open-loops-system.md`, the grading stage at `prompts/open-loops/open-loops-roll-up.md`, and the deeper senior-strategist reasoning in `global/knowledge/creative-strategy/creative-strategy-fundamentals.md`, loaded via expertise-routing. What follows is the foundational teaching every doc author needs to internalize.
