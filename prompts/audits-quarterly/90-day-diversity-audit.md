...
Read `global/knowledge/creative-strategy/ad-formats/` before classifying the account's format mix or recommending format tests. The taxonomy is a shared language for format awareness, not a cage. If the account's execution lives in a gray area, describe the execution first and then name the closest strongly supported format.
