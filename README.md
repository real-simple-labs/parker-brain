...
> If you were asked to set up, build, or onboard a brand's brain from this repo, that file is the canonical, executable, step-by-step method — read it in full and follow it exactly, including its Phase 0 (work inside the brand's own synced folder, the one the Parker Desktop app created and watches — never in this repo, and never through git). This README is the human overview; the runner is the procedure. If the `/set-up-brain` skill is available in your session, invoke it — it runs the same runner and owns the experience around it.
...
- **Its own repo, separate from this one.** The brand's folder is the whole workspace: the brand's data sits flat at its root, nothing brand-specific ever lands in the factory, and the Parker Desktop app keeps it backed up and synced — the agent never runs git in it.
- **Skills register the moment the folder opens.** The executable skills (craft + routines) are copied into the brain's `.claude/skills/` — the one directory Claude Code loads skills from — so they work on any machine the folder syncs to.
...
- `.claude/output-styles/` - Parker's voice as a Claude Code output style (`parker.md`), switched on by `"outputStyle": "Parker"` in the committed `.claude/settings.json`. It is injected into the system prompt itself, so Parker talks plain and warm from the first message — here and in every shipped brand brain — instead of sounding like a dense terminal.
- `.agents/skills` + `AGENTS.md` - the OpenAI Codex surface. The symlink lets Codex discover the same skills; `AGENTS.md` is its entry point. Brand brains carry a fuller twin (`.codex/` hooks, the voice through the brain's own `AGENTS.md`) — `system/codex-support.md` is the contract for all of it.
- `tests/` - runtime hook regressions (`python3 -m unittest discover -s tests -v`) and an optional offline Codex integration probe (`python3 tests/probe_codex_runtime.py`). CI runs the unit suite on Linux, macOS, and Windows; `system/codex-support.md` records the runtime contract.
...
- `scripts/scaffold-brain.py` + `system/brain-scaffold.md` - the brain scaffold: every file a new brand brain starts with, built with no AI. The build runs it in the brand folder; `.github/workflows/release-scaffold.yml` attaches the same scaffold to every release as `brain-scaffold.json` so Parker's backend can create a scaffolded brand repo with three GitHub write calls, plus preflight reads. A `.scaffolded` file marks a brain as set up with nothing built yet; the `pull-log` hook takes it off when the build reports its first finished phase after Phase 0. What a new brain gets is defined by the bundle map in `scripts/sync-executable-layer.py` plus `SEEDS` in the scaffold script; a file every brain needs goes in one of the two.
