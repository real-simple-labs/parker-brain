...
`parker-brain` is meant to be cloned as the read-only factory. **A brand brain you build from it lives in its own separate repository — you do not build on top of this repo.** Keep a working folder with three things in it: a top-level `AGENTS.md` that orients any AI agent opening the folder (pointing at this README and at the `setup_parker_brain` tool's instructions), a `factory/` folder where this repo is cloned, and a `<brand name>-brain/` folder where the brand's own repo is cloned. That brand repo is provisioned by the **`setup_parker_brain` tool on the Parker MCP**: it creates the brand's private GitHub repo — or retrieves the existing one — and returns short-lived credentials to clone and push with, so you never create the repo by hand. If the retrieved repo already contains a built brain, use it as-is; if it's empty, the onboarding runner generates the knowledge and saves it there. Commit and push often, as soon as you make any changes. Do a `git pull` in `<brand name>-brain/` before doing updates and `git pull` in `factory/` as well. This clone supplies the prompts, skills, methodology, and craft layer, and the brand repo is the product. Onboarding copies that whole method into the brand repo — the skills (craft + routine) into `.claude/skills/`, the one directory Claude Code loads skills from so they register the moment the brain is cloned, and the rest under `parker-system/` (`prompts/`, `creative-strategy-context/`, `system/`) — so the brand brain is self-contained: it can refresh and rebuild its own docs by re-running the exact prompts that generated them, and execute craft through its own skills, with no link back to this factory. `prompts/onboarding-runner.md` is the executable cold-start sequence and `prompts/README.md` is the why behind it.
