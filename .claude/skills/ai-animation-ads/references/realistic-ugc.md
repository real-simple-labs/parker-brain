...
- **Dialogue is speech, not copy (disfluency default):** write UGC lines the way a person actually talks — ums, likes, false starts, self-interrupts, trailing pauses ("Okay, um… it does kind of look like swamp water. Not gonna lie."). A grammatically clean line spoken aloud is an AI tell by itself. Protect the disfluencies through generation: "she says exactly these words and nothing else, including the ums and hesitations exactly as written." Disfluencies count toward the word budget but speak fast — time the line aloud, not by word count alone.
