...
### 2. DIALOGUE — written FROM the studied corpus, at reference density
Structure rule: **the creator talks wall-to-wall; actions ride UNDER speech, never instead of it.** Write each 8s segment as ~18–21 words of continuous casual narration (≈2.4 w/s), mostly 3–8 word breaths.
Voice rule — the harder one, learned across five owner strikes: **structure in band is not voice.** Draft every line from `native-speech.md` (the studied-corpus doctrine) and gate against its pre-flight nativeness checklist: first-person density ≥1 per ~10 words (zone-legal classes — reactions, choices, curiosity, present feelings), every reaction argues with a voiced prior expectation, facts read off visible objects, lived-life anchors, intensifiers (actually/literally/just/honestly) not scripted filler sounds, vague-topic gestures allowed, benign sloppiness allowed, the narrator test on every line (a line speakable by someone not in the room is copy, not speech), no repeated segment openers, no crafted images/tricolons/joke shapes in mouths.
**Mechanical validation FIRST:** word counts per breath vs the spec distribution, projected density (words ÷ segment seconds ≥ 2.2 w/s), plus the nativeness checklist. Out of band on either = rewrite before judgment.
Then **REVIEW 2 — SCRIPT** (agent pass): creative-voice-review + ai-writing-tells + presenter zone + claims, WITH native-speech.md in hand. Both checks pass before a credit moves.
...
Every take: **transcript verification word-for-word** (a doubled word = prefer the double-free sibling take; a forced excision must be dressed per the motivated-cut law, never cut "invisibly") + **label OCR at zoom on ripped frames** (label drift mid-motion kills the take) + action-on-words check + **glitch scan** (frame-step the full take for baked-in jumps — adjacent-frame delta spikes away from planned cuts; generative sources ship glitches inside "single takes"). Two traps this gate exists for: (a) gate the **SELECTED** take — zooming take B and shipping take A is how a drifted pack ships; (b) words are not enough — the pack **design** can drift while every word stays spelled (an invented accent color, a changed lockup). So the gate evidence includes a **cross-clip pack strip**: one pack crop per selected clip, side by side with the packshot; any visible design delta = re-roll that clip. Then upscale winners (bytedance `ugc` preset → 1080p) — and remember the upscaler denoises, so the grade pass comes AFTER.

### 6. EDIT — to the parity spec, not to a rhythm of its own
- **THE MOTIVATED-CUT LAW (glitch killer):** every cut carries a visible delta — prop state, framing punch, angle, location, or on-screen text changing WITH it. A cut with no delta reads as a stutter, even mid-phrase cuts are native WITH a delta (winners cut inside "I'm actually in shock" while the bag rotates). Excisions (dedup, gap-trim) are dressed as motivated jump cuts (3–6% punch + caption swap) or relocated to a state change — never "invisible."
- **Density first:** cut silence gaps and duplicated phrases so assembled speech density lands ≥ ~88%. Trim any low-density take hard (a 6-word take occupies ~3s, not 8). Prefer excising at motivated cuts over in-shot speed ramps; ramps only for a referenced action that must survive, under ~2x.
- **Cadence:** register-specific — inherit the reference's (selfie/unboxing ≈1.5–2.5 jump cuts/10s with 8–12s uncut stretches fine; errand-vlog ≈5–8 b-roll cuts/10s under continuous VO; expert ≈ zero body cuts). Do NOT add punch rhythm beyond the reference's own grammar.
