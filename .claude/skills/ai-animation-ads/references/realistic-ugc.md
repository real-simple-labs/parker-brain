...
- **Succinct or it's written (owner law, 4th dialogue correction):** no spoken breath over ~8 words — a segment is 2-3 short bursts, never one composed sentence. In demo/unboxing beats the words follow the HANDS item by item ("The bag. This jar thing... an actual metal scoop?"), never an inventory sentence. Crafted images/metaphors are banned from mouths — if a phrase would look good on a static, it's copywriting, not speech. Sentence-initial rhetorical scaffolding ("So if..., this is what...") is ad structure; speech jumps, it doesn't argue.
