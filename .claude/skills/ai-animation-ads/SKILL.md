...
| U. Realistic-UGC track | Photoreal creator-style UGC: whole-reference segment adaptation → plate per segment → Omni clips with native dialogue → captioned assembly | THE THREE REVIEWS: strategy (mechanic availability + vault-verified SKUs) -> script (voice + AI-tells, drafted from native-speech.md) -> edit (ugc-edit-reviewer on the cut, motivated-cut law), plus frame/transcript/voice gates | realistic-ugc.md + native-speech.md |
...
- **`creative-strategy-context/ai-video-ad-structure.md`** (repo-root canon) — the ad-structure doctrine every video build follows before any frame or clip work: runtime selection from the message, the story architectures, the natural product handoff, pacing laws, proof hold, audio spine, continuity across generations. The script and storyboard stages structure the ad per this doc.
...
- **native-speech.md** — how real ads talk and cut, dissected from a live corpus of winning ads across formats: the seven speech laws (first-person density, expectation arcs, object-sourced facts, lived-life anchors, the authenticity lexicon, vague gestures, benign sloppiness), the per-format register map (creator/vlog/expert/celebrity/skit/VO-demo/statics), the motivated-cut edit law, and the pre-flight nativeness checklist. Read it before writing ANY spoken dialogue in ANY format, and before planning any edit
