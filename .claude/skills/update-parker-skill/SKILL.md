...
- **Ship-list check.** Knowledge docs and prompts reach every brain through the `parker-system/` mount. Anything a brain must carry as its own copy (a skill, an agent, a hook, a script) has to be in the bundle map in `scripts/sync-executable-layer.py`, the one list both the scaffold (new brains) and `/update-brain` (standing brains) read; check it covers the new path and update the runner's description of what ships (Phase 0 step 5) in the same pass. A brand-owned starting file goes in `SEEDS` in `scripts/scaffold-brain.py` instead, and already-built brains get it only through a real migration step (`migrations/README.md`). Miss both and the skill works in the factory and points at nothing in every brand brain. `system/brain-scaffold.md` has the rules; `tests/test_scaffold_brain.py` proves the result.
