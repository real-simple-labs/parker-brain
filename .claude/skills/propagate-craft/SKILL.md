...
## The pull-based half now exists: /update-brain

Every brain now ships the `update-brain` routine (weekly): it fetches the factory itself, compares the methodology layers and the brain's own canonical build, and offers every gap to the team — never applying without their yes, remembering declines. That covers the pull side; this push script remains useful for factory-initiated sweeps of already-present files.

## Better future: a release-pinned model
