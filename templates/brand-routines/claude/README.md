...
- **`hooks/git-guard.py`** — the `PreToolUse` guard on Bash. On a repo whose origin is under the `parker-brain` org it blocks the wrong git moves — `gh` (the user's personal login), `push`/`pull`/`fetch` without Parker's credentials, any force-push, a clone missing `--recurse-submodules` — and its block message teaches the two-line fix (`setup_parker_brain` → `git remote set-url origin <fresh url>` → retry). Self-hosted repos (any other origin) pass through untouched, as do mount operations (`git -C parker-system …`). Fails open internally; its settings entry must never get the `2>/dev/null || true` wrapper the other hooks use, because exit code 2 and stderr are the mechanism. The procedure it enforces is the `save-brain` skill.
- **`hooks/commit-guard.py`** — the `Stop` guard. When a turn is about to end on a managed repo with uncommitted changes or unpushed commits, it blocks the stop once and tells the agent to finish the save-and-push loop — without asking the user. `stop_hook_active` prevents loops (the second stop passes with a note). Same no-wrapper rule as git-guard; fails open internally.
- **`skills/`** — `.claude/skills/` is where Claude Code loads skills from, so **all** of the brain's skills live here. Alongside the routines listed below sit the **craft skills** (scriptwriting, hooks, headlines, iterations, ad-account-analysis, ai-ad-generation, the open-loops pipeline, and the rest), copied in during onboarding so the brand `CLAUDE.md` can route execution through `.claude/skills/<skill>/` and they register the moment the brain is cloned. The routines, each a self-contained skill (invoke as `/refresh-context`, `/dream`, etc.):
  ...
  - `save-brain` — the required git procedure for every push, pull, clone, or sync: credentials live in the remote (from `setup_parker_brain`; never the user's login, never `gh`), plain `git pull --rebase origin main` / `git push origin main` with a two-line credential refresh on auth errors, submodules always, conflicts resolved best-effort keeping both sides, every change committed and pushed the moment it's done — without asking. The `git-guard` and `commit-guard` hooks enforce it. On-demand, not scheduled.
