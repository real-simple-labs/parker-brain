---
name: dream
description: Parker's dreaming run for the brand — the planning half of the living layer. Reads the day's comms (the conversations the user had with Parker first, then any connected team comms) against the vault and returns proposals across five buckets — context updates, skill improvements, schedules to create, new ideas to research, new open loops — into dreaming/proposals/pending. Captures verbatim; proposes, never applies; promotion runs through self-improve with the human in the loop. Use daily as a scheduled routine, or when asked to "dream", brainstorm overnight, or "what were you thinking about".
---
