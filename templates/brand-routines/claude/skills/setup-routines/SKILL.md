...
One boundary to state plainly if it comes up: these schedules are Claude Code cloud agents and can only be armed from a Claude Code session. In OpenAI Codex the routine skills themselves still work on demand (`$refresh-context`, `$dream`, …), but nothing runs on a clock from there — a Codex-only team runs them by hand or wires their own external cron — and for cron, someone must first open the brain in the Codex TUI once and approve the hooks, because headless `codex exec` silently skips hooks that were never approved (`parker-system/system/codex-support.md`).
