...
Loop safety: when `stop_hook_active` is true in the payload, a Stop hook
already blocked this turn once — we let it end (whatever went wrong needs the
user, not an infinite loop) and just describe the leftover state.
...
def main() -> int:
    ...
    if data.get("stop_hook_active"):
        # Already blocked once this turn; don't loop. Surface the state instead.
