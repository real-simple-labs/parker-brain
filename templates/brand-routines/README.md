...
- **`claude/`** → stamped to the brand brain's **`.claude/`**. (Named `claude/` without the dot on purpose, so Claude Code does not treat the factory's own template as active config. The runner renames it to `.claude/` on stamp.)
  - `settings.json` + `hooks/` — the committed guardrails and hooks (brand-agnostic; Claude Code hooks, no relation to ad hooks). settings.json carries the deny rules that keep the `parker-system/` mount read-only, plus five hooks: `hooks/session-start.py` (SessionStart — catches an un-initialized mount, and on the standard layout runs the start-of-session `git pull --rebase origin main` itself when the tree is clean, reporting loudly when it can't), `hooks/craft-context.py` (UserPromptSubmit — injects the live craft catalog from `parker-system/creative-strategy-context/expertise-routing.md` into every turn, with the sources-receipt rule and a static fallback), `hooks/pull-log.py` (PostToolUse — the session pull log the grounding gate verifies), `hooks/git-guard.py` (PreToolUse on Bash — on parker-brain-org repos it blocks `gh`, credential-less git network commands, force-pushes, and submodule-less clones, teaching the `setup_parker_brain` flow), and `hooks/commit-guard.py` (Stop — blocks ending a turn with uncommitted or unpushed changes on managed repos, once per turn). The two guards are registered without the `2>/dev/null || true` wrapper because their exit codes and stderr are the mechanism.
  ...
  - `skills/` — the bundled skills, self-contained (no `parker-brain/...` paths at runtime). The first four are the scheduled routines; the last two are on-demand helpers:
    ...
    - `save-brain` — the required git procedure: credentials live in the remote (from `setup_parker_brain`; never the user's login, never `gh`), plain `git pull --rebase origin main` / `git push origin main` with a two-line credential refresh on auth errors, submodules always, conflicts best-effort, every change committed and pushed immediately without asking, and the self-hosted exception detected from the origin URL. The `git-guard` and `commit-guard` hooks enforce it; this skill teaches it.
