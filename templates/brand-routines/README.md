...
- **`claude/`** → stamped to the brand brain's **`.claude/`**. (Named `claude/` without the dot on purpose, so Claude Code does not treat the factory's own template as active config. The runner renames it to `.claude/` on stamp.)
  - `settings.json` + `hooks/` — the committed guardrails and hooks (brand-agnostic; Claude Code hooks, no relation to ad hooks). settings.json carries the deny rules that keep the `parker-system/` mount read-only, plus four hooks: `hooks/session-start.py` (SessionStart — catches an un-initialized mount, and on the standard layout runs the start-of-session `git pull --rebase origin main` itself when the tree is clean, reporting loudly when it can't), `hooks/craft-context.py` (UserPromptSubmit — injects the live craft catalog from `parker-system/creative-strategy-context/expertise-routing.md` into every turn, with the sources-receipt rule and a static fallback), `hooks/pull-log.py` (PostToolUse — the session pull log the grounding gate verifies), and `hooks/git-guard.py` (PreToolUse on Bash — on parker-brain-org repos it blocks `gh` aimed at this repo (other targets pass), any command carrying the raw token, credential-less git network commands, force-pushes, and submodule-less or helper-less clones, teaching the `setup_parker_brain` credential-file flow from `/save-brain`; registered without the `2>/dev/null || true` wrapper because its exit code and stderr are the mechanism).
  ...
  - `skills/` — the bundled skills, self-contained (no `parker-brain/...` paths at runtime). The first four are the scheduled routines; the last two are on-demand helpers:
    ...
    - `save-brain` — the required git procedure: credentials live in the local file `.git/parker-credentials` (the token from `setup_parker_brain`, written with the Write tool — never inside a shell command, never the user's login, never `gh`), origin on the plain URL, plain `git pull --rebase origin main` / `git push origin main` with a re-write-the-file refresh on auth errors, submodules always, conflicts best-effort, every change committed and pushed immediately without asking (unless the user explicitly says not to), everything landing on `main` — no branches or PRs, even when a hosted session assigns a working branch — and the self-hosted exception detected from the origin URL. The `git-guard` hook enforces the credential rules; the commit-and-push discipline lives in this skill and the brand `CLAUDE.md`.
...
- **`codex/`** → stamped to the brand brain's **`.codex/`** (same dot-rename trick as `claude/`). The OpenAI Codex twin of the guardrail layer: `config.toml` wires the same four hooks (delegating to the same `.claude/hooks/` scripts) plus a `mount-guard` PreToolUse hook that stands in for the deny rules, which Codex lacks. Change a hook in `claude/settings.json`, change it here in the same PR — `parker-system/system/codex-support.md` is the contract.
