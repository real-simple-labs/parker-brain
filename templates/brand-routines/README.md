...
- **`claude/`** → stamped to the brand brain's **`.claude/`**. (Named `claude/` without the dot on purpose, so Claude Code does not treat the factory's own template as active config. The runner renames it to `.claude/` on stamp.)
  ...
  - `skills/` — the bundled skills, self-contained (no `parker-brain/...` paths at runtime). The first four are the scheduled routines; the last two are on-demand helpers:
    ...
    - `save-brain` — the required git procedure: auth is the machine-level Parker sync helper (`npx @heyparker/sync setup`, once per machine — per-operation server-minted credentials, never the user's login, never `gh`, no tokens or credential files anywhere), origin on the plain URL, plain `git pull --rebase origin main` / `git push origin main`, legacy per-repo credential wiring cleaned on sight, submodules always, conflicts best-effort, every change committed and pushed immediately without asking (unless the user explicitly says not to), everything landing on `main` — no branches or PRs, even when a hosted session assigns a working branch — and the self-hosted exception detected from the origin URL. The `git-guard` hook enforces the credential rules; the commit-and-push discipline lives in this skill and the brand `CLAUDE.md`.
