...
- **`claude/`** → stamped to the brand brain's **`.claude/`**. (Named `claude/` without the dot on purpose, so Claude Code does not treat the factory's own template as active config. The runner renames it to `.claude/` on stamp.)
  - `settings.json` + `hooks/` — the committed guardrails and hooks (brand-agnostic; Claude Code hooks, no relation to ad hooks). settings.json carries the deny rules that keep the `parker-system/` mount read-only, plus four hooks: `hooks/session-start.py` (SessionStart — catches an un-initialized mount and reminds about pulling first), `hooks/craft-context.py` (UserPromptSubmit — injects the live craft catalog from `parker-system/creative-strategy-context/expertise-routing.md` into every turn, with the sources-receipt rule and a static fallback), `hooks/pull-log.py` (PostToolUse — the session pull log the grounding gate verifies), and `hooks/git-guard.py` (PreToolUse on Bash — on parker-brain-org repos it blocks `gh`, credential-less git network commands, force-pushes, and submodule-less clones, teaching the `setup_parker_brain` flow; registered without the `2>/dev/null || true` wrapper because its exit code and stderr are the mechanism).
  ...
  - `skills/` — the bundled skills, self-contained (no `parker-brain/...` paths at runtime). The first four are the scheduled routines; the last two are on-demand helpers:
    ...
    - `save-brain` — the required git procedure: Parker's short-lived credentials via `setup_parker_brain` (never the user's login, never `gh`), explicit `git push origin main` bracketed by token-in/token-out on the remote, submodules always, conflicts best-effort, and the self-hosted exception detected from the origin URL. The `git-guard` hook enforces it; this skill teaches it.
