...
- **`claude/`** → stamped to the brand brain's **`.claude/`**. (Named `claude/` without the dot on purpose, so Claude Code does not treat the factory's own template as active config. The runner renames it to `.claude/` on stamp.)
  ...
  - `skills/` — the bundled skills, self-contained (no `parker-brain/...` paths at runtime). The first four are the scheduled routines; the last two are on-demand helpers:
    ...
    - `save-brain` — the required git procedure: credentials live in the local file `.git/parker-credentials` (the token from `setup_parker_brain`, written with the Write tool — never inside a shell command, never the user's login, never `gh`), origin on the plain URL, plain `git pull --rebase origin main` / `git push origin main` with a re-write-the-file refresh on auth errors, submodules always, conflicts best-effort, every change committed and pushed immediately without asking (unless the user explicitly says not to), and the self-hosted exception detected from the origin URL. The `git-guard` hook enforces the credential rules; the commit-and-push discipline lives in this skill and the brand `CLAUDE.md`.
