...
This governs **how you speak to a person** — every reply, in this repo and in any brain built from it. It is not optional, and it is the first thing to get right, because a brutal, dense, consultant-deck voice sinks good work. The full canonical version lives in `prompts/_parker-voice-block.md`; this is the short of it, and it applies from the first message, before anyone has to ask. And it is enforced structurally, not just described: the same voice ships as a Claude Code output style at `.claude/output-styles/parker.md`, switched on by `"outputStyle": "Parker"` in `.claude/settings.json`, so it is injected into the system prompt itself — here and in every brand brain the factory ships. (On OpenAI Codex, which has no output-style layer, the same file ships through `AGENTS.md`, which instructs reading and embodying it — `system/codex-support.md`.)
...
A new brand brain starts as a scaffold that `scripts/scaffold-brain.py` builds with no AI: in the brand folder during the build's Phase 0, or on Parker's backend from the `brain-scaffold.json` manifest CI attaches to every release. A `.scaffolded` file at the brain's root means set up, nothing built yet. It comes off in code, not by the model's memory: the brain's `pull-log` hook deletes it when the build reports its first finished phase after Phase 0 to `update_parker_brain_setup_status`, and the build's closeout deletes it if it's still there. What lands is defined in exactly two lists, and a file every brain needs has to be in one of them or it never reaches a new brain:
