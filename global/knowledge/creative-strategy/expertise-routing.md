...
Paths are relative to `creative-strategy-context/` in whichever prompt tree is running (`parker-v2/` or a brand brain's `parker-system/`). Where a brand lens overlay exists (`_<brand>-lens.md`), load it after the generic docs — it bends the rules for this brand.
