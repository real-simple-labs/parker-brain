...
- **`commit-guard.py` (new Stop hook).** Ending a turn on a managed repo with uncommitted changes or unpushed commits gets blocked once, with the save-and-push loop in the message. Loop-safe twice over: `stop_hook_active` caps it at one block per turn, and a remembered fingerprint of the unsaved state caps it at one block per state — a persistent git error goes quiet instead of taxing every turn, and new work re-arms the guard. Registered in the shipped `settings.json` without the output-swallowing wrapper.
