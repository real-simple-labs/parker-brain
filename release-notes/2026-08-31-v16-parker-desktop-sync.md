...
- **`git-guard.py` simplified** (routine bundle) — on managed repos it now blocks the git commands that move history, the network, or the working tree against the brand repo (push, pull, fetch, clone, commit, rebase, merge, reset, restore, checkout, clean, stash, remote set-url) plus `gh` aimed at this repo, because two sync engines racing over one folder is how work gets destroyed. Read-only git (status, log, diff) passes, and so do mount operations (`git -C parker-system …`, `git submodule …`) — local, credential-free, and still the agent's job for `/update-brain`'s pin move and the empty-mount heal.
