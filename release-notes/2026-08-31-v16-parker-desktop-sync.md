...
- **`save-brain` rewritten** (routine bundle) — from a git procedure to a one-step truth: write the file, the app syncs it. Covers finding the brain folder via **`~/.parker/workspace.json`** — the pointer file the app maintains, whose `root` is the parker folder with one subfolder per brand (fallback is asking the user) — what to do when the folder isn't syncing (point at the app, or a technical team wires its own git), and the self-managed exception, still detected from the origin URL.
- **`git-guard.py` simplified** (routine bundle) — on managed repos it now blocks the git commands that move history, the network, or the working tree against the brand repo (push, pull, fetch, commit, rebase, merge, reset, restore, checkout, clean, stash, cherry-pick, revert, am, remote set-url, and branch mutations — delete, move, force, copy), cloning any managed-org URL, plus `gh` aimed at this repo (mutating `gh repo` subcommands included), because two sync engines racing over one folder is how work gets destroyed. Read-only git (status, log, diff) passes, so do plain submodule commands and clones of non-managed URLs (the public factory included), and mount operations (`git -C parker-system …`) stay the agent's job for `/update-brain`'s pin move and the empty-mount heal.
...
- **`system/brain-git-sync.md` → `system/brain-sync.md`** — the maintainer doc rewritten for the new model, including the open cross-team duties (the app maintains `~/.parker/workspace.json`; the old tool message still teaches the retired flow until the tool is retired).
