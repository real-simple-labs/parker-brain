...
- **`retro.md` has no generating prompt yet.** It is referenced across the Phase-3 docs as the per-round retrospective that feeds the next sprint's sizing, but nothing in Phase 3 produces it — today it is written at the monthly creative-retro cadence named in `persona-research-and-creative-strategy-process.md`. A dedicated retro prompt (round results → validated hypotheses + dead ends) is a sensible future Phase-3 add; flagged so the artifact is not mistaken for auto-generated.
