...
### `product-launch/`
Launch creative for an established brand's new product or SKU — the brief where the persona, the proof, the brand's authority, and the ad's strategic job are all different from routine performance work. Classifies the launch before writing: scope gate (a restock or "limited edition" wrapper on an unchanged SKU is not a launch), then audience fit and type of newness, producing one of three scenarios — depth play (existing persona, new attribute), bridge and expand (existing persona, new format or bundle), audience expansion (new persona, gated on persona work). Nine processes: the required account diagnostic, the three scenario playbooks, proof substitution, phase sequencing, special contexts (gifting, giveaway, multi-video through-lines), deliverable specs, and edge-case routing. Loads `new-product-launch-creative.md` as its reasoning layer plus `killer-performance-ads.md` for the canonical awareness stages, and hands the actual writing to `scriptwriting`, `hooks`, or `headlines` while carrying the classification into the handoff. Runs both ship gates.
...
| 2026-07-21 | **Launch creative becomes its own discipline.** New knowledge doc `creative-strategy-context/new-product-launch-creative.md` (why a launch is a different brief than routine performance work, the newness taxonomy, the three launch scenarios, how brand authority transfers and doesn't, honest proof when the SKU has none) and new craft skill `product-launch/` with `strategy.md` plus nine processes. Promoted from a team context doc. The core move is that classification precedes writing: a new color, a new format, and a new audience are three different briefs that get confused constantly, and the wrong scenario produces the wrong script no matter how well written. Scope is drawn tight — a restock, re-release, or "limited edition" wrapper on an unchanged SKU is not a launch and routes out; a genuinely new colorway inside a limited drop is Scenario A. Awareness stages are referenced from `killer-performance-ads.md` rather than restated, with one scoped override named explicitly: the warm-launch phase defaults to most-aware against that doc's solution-aware default, because an ad to the brand's own buyer list genuinely addresses a most-aware audience. Output-contract precedence defers the deliverable's shape to the host craft skill (`scriptwriting`, `hooks`, `headlines`) and adds the Launch Context Summary on top. Two dangling invocations from the source doc resolved: "Persona Development" now routes to the `prompts/personas/` tree, "Script Adaptation" to `scriptwriting`'s `adapt-existing-script`. Gifting carries an interim treatment pending a dedicated Gifting Ads doc. Client names in the source doc were generalized before promotion. Ships to brand brains paired with its method doc via the onboarding-runner ship step and the propagate deliberate-adds list. |
