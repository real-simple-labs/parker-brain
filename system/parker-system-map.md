...
| 2026-08-04 | **The agent leaves the credential loop (v15).** Git auth for managed brains moves to the machine-level `@heyparker/sync` credential helper: a one-time `npx @heyparker/sync setup` signs the user into Parker and wires a global git config scoped to the `parker-brain` org; from then on git itself fetches a short-lived, single-repo, server-minted credential per operation, and no token ever enters the agent's context, a shell command, or a file. The v8/v10 per-repo layout (`.git/parker-credentials` + repo-local two-entry `credential.helper`) is retired and now actively harmful — stale local wiring shadows the helper — so `session-start.py` cleans it deterministically on auth failure, `save-brain` teaches clean-on-sight, `git-guard.py` re-keys its auth check to the helper (legacy layouts accepted mid-transition), and the settings drop the credential-write pre-approvals for an `npx @heyparker/sync setup` allow. The onboarding runner's clone step becomes a plain `git clone --recurse-submodules`. Cross-repo: mevin2 gains `/api/git-credential` (brand-membership-checked, repo-downscoped GitHub App mints); `setup_parker_brain` should return only the plain URL once standing brains migrate. Ships as `v15` with real-step `migrations/v15.md` (one-time machine setup + per-repo legacy cleanup); rationale in `system/brain-git-sync.md`. |
