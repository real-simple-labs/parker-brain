...
| Date | Change |
|---|---|
| 2026-09-15 | **Clashes are combined in the files (v22).** Parker Desktop (the release after 0.15.4) settles a clash by merging the shared version into the folder itself and leaving both versions in each file between `<<<<<<<` and `>>>>>>>` marks; the assistant's request is to edit the files until no mark is left, and the app saves and shares the result. `git-guard.py` lets `git add`, a plain `git commit` and `git merge` through so a hand-finished combine is never blocked halfway (`push`, `pull`, `fetch`, `rebase`, `commit --amend`, `merge --abort` stay blocked). `save-brain` gains "When Parker paused the folder on a clash"; the brand `AGENTS.md`, `session-start.py` and the `CLAUDE.md` template carry the one-line rule; `system/brain-sync.md` gains the "Clashes" section. No-op `migrations/v22.md`. |
