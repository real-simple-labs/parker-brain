...
Generates a shareable weekly visual ad-performance report for any brand on the Parker MCP, published as a claude.ai Artifact. Pulls two windows of top ads (report period + prior, for WoW trend and churn), detects the brand's new-customer purchase event for per-ad and per-tag NC-CPA, and grades everything with the ad-account method — slow-burner / high-riser / fatigued classification, promo caveats, the small-spend-high-ROAS trap. Output is a fixed light-mode card-gallery template (`template.html`, the UI contract): brand bar, KPI row, tag-economics layer (spend share vs what it earned, by emotion / format / awareness), theme galleries with embedded real creative (base64 data URIs — the artifact CSP blocks remote media), an iterate / watch / review verdict strip, and number-backed takeaways. Loads `creative-strategy-context/ad-account-analysis.md` and `selecting-ads-to-iterate-on.md` when on disk; carries the distilled rules standalone.
