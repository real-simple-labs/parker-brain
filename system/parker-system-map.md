...
| 2026-07-13 | **Credential-write hardening (v10).** The first field weekend showed the v8 credential Write failing intermittently: the Write tool refuses to overwrite an unread file (broke every hourly refresh), and the safety classifier probabilistically blocks token writes it deems un-requested. Fixes: `rm -f .git/parker-credentials` before every re-Write (skill, session-start message, guard text); `permissions.allow` rules for the credential file in the bundle's `.claude/settings.json` — pre-approval by committed configuration, checked before the classifier, which is also what lets headless scheduled runs write credentials with nobody watching; a refusal playbook in `save-brain` (read-guard → rm-first; classifier → ask the user in plain words via the question tool and retry on their yes; headless → commit locally, end loudly); and the session-start auth recovery gated on the managed-origin test so self-hosted brains get pointed at their own auth, never Parker's flow. Deliberate non-change: `parker-credentials` keeps its honest name — content is what classifiers read, and an innocuous name reads as obfuscation. `migrations/v10.md`: no-op unless the team customized `settings.json` (manual allow-block merge shown). |
