...
- `system/brain-scaffold.md` — what a new brand brain starts with and how it gets there: the two lists that define it (the bundle map and `SEEDS`), the `.scaffolded` marker, the release manifest CI attaches to every release, and the backend's three REST calls
...
| Date | Change |
|---|---|
| 2026-09-25 | **A new brain starts as a scaffold, built by a script (v23).** `scripts/scaffold-brain.py` sets up an empty brand brain with no AI: the bundle-map method copies, the brand-owned seeds (the brand `CLAUDE.md` in its not-built-yet form, a starter `README.md`, the six living-layer folder READMEs from `templates/brand-scaffold/`, the brand lens, the running notes, `parker_config.json`), the `.agents/skills` symlink, and a `.scaffolded` marker. The runner's Phase 0 runs it in place of the hand copy; `.github/workflows/release-scaffold.yml` attaches the same scaffold to every published release as `brain-scaffold.json`, so Parker's backend can create a scaffolded brand repo with three write calls, plus preflight reads, and no git. The marker is the one "not built yet" signal: the runner and `/set-up-brain` route a marked folder as a cold start with setup done, the session-start hook and `/get-started` say so, apps read its presence from the root listing, and it comes off in code: the brain's `pull-log` hook deletes it when the build reports its first finished phase after Phase 0 to `update_parker_brain_setup_status`, with closeout as the fallback. The runner stops swapping the brand name into copied method files, which froze them against `/update-brain`. Contract: `system/brain-scaffold.md`. Rationale: the setup was 160-odd file operations of model time with no judgment in them, a brain can now exist (and fill by conversation) before anyone builds it, and the frontend needs a cheap built-or-not signal. |
...
`system/brain-scaffold.md` owns what a new brain starts with. `scripts/scaffold-brain.py` reads the bundle map in `scripts/sync-executable-layer.py` (method copies, re-synced on every pin bump) and its own `SEEDS` (brand-owned starting files, written once); a file every brain needs belongs in one of the two. The onboarding runner's Phase 0 step 3 runs `init`; CI runs `manifest` on every release; the backend applies the manifest. `.scaffolded` marks a set-up brain with nothing built yet; the `pull-log` hook deletes it when the build reports its first finished phase after Phase 0, and closeout deletes it if it's still there.
