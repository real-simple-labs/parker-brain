...
| 2026-08-24 | **The away digest — session-start catches the returning user up.** `session-start.py` (the copied hook bundle) now keeps a per-clone stamp — `.git/parker-last-session`, start time plus the HEAD that session saw, inside `.git/` so it never commits — and when a session opens after a 12+ hour break it hands the model an AWAY DIGEST: the commits teammates and scheduled routines landed in the brain since the user was last here, so the recap ("here's what changed while you were away") opens the conversation before the actual request. Division of labor is deliberate: the script does only the mechanical filters (the gap, the commit range, the user's own commits dropped by this clone's git identity, a rewritten-history ancestor guard, a 20-commit cap) and the model keeps the judgment call — recap in a couple of plain sentences with no git talk, reading the changed files first when subject lines aren't enough, or skip entirely when it's all housekeeping, because a recap of nothing is noise. Standard-layout brains only, and only after a successful start-of-session pull (so the digest recaps what actually just landed; a failed or skipped pull leaves the stamp put and the whole span digests once syncing works again); silent on first session, short gaps, own-only changes, rewritten history, and any error — the digest is a nicety, never a blocker. Docs updated in the same pass: the settings.json `$comment`, `templates/brand-routines/claude/README.md`, `system/brain-git-sync.md`. Bundle-only change — `sync-executable-layer.py` delivers it on the next pin bump; the next release's migration is a no-op. |
