...
### `idea-bank/` · `[built]`
The brand's idea memory — reusable ad ideas, concepts, hooks, and formats worth stealing, captured as one folder per idea under `entries/` with an `index.md` roll-up. Scaffolded across the early brand brains, populated for some and empty for others. This is idea memory, not strategy: entries carry the novelty check, the reasoning, the why-now, and the source examples that teach the idea, and never carry brand-adaptation advice. New entries land as `[~]` proposed and become trusted taste only on Jimmy's approval. The reusable cross-brand counterpart lives in the knowledge tier at `creative-strategy-context/parker-taste/idea-bank/`.
