...
| 2026-08-31 | **Parker Desktop owns the sync (v16).** The brain's whole git layer is retired: the Parker Desktop app now creates the brand's repo (its set-up-a-repository feature, which then opens a Claude session in the synced folder), syncs the folder both ways, and makes saving mean nothing more than writing files. No provisioning tool call, no credentials, no pulls or pushes — agents never run git against a brand repo; the one carve-out is the method mount (`git -C parker-system …`, `git submodule …`), still local and credential-free for `/update-brain`'s pin move and the empty-mount heal. `save-brain` rewritten around the app (folder discovery via `~/.parker/workspace.json`, the pointer file the app maintains whose `root` holds one subfolder per brain; not-syncing → point at https://app.heyparker.ai/dashboard/parker-desktop or a technical team's own git), `git-guard.py` now blocks the brand-repo git commands that move history/network/working tree plus `gh`, while passing mount ops and read-only git, `session-start.py` drops the pull and credential recovery, the settings lose the credential allow rules, and the onboarding runner + `/set-up-brain` + factory README start from the app-created folder and end by confirming the sync instead of pushing. `system/brain-git-sync.md` renamed to `system/brain-sync.md` and rewritten. Ships as `v16` with real-step `migrations/v16.md` (delete the stale credential file, introduce the team to the app honestly, strip the retired allow rules from team-edited settings, refresh the brand `CLAUDE.md` git sections). |
| 2026-09-02 | **OpenAI Codex support (v17).** The whole runtime gains a Codex twin, contract in `system/codex-support.md`, everything verified against a real Codex 0.147.0 install rather than secondhand docs. Codex adopted Claude Code's hook wire format nearly verbatim, so the four brand hooks run unchanged from `.codex/config.toml` (stamped from `templates/brand-routines/codex/`), with two envelope-only adaptations: `git-guard.py --codex` blocks via the JSON permissionDecision deny instead of exit-2/stderr, and the new `mount-guard.py` PreToolUse hook keeps `parker-system/` read-only because Codex has no per-path permission deny rules. Skills need no second copy — Codex discovers them from `.agents/skills`, a committed symlink to `.claude/skills` (verified working), in the factory and in every brand. The voice ships through a new brand-neutral root `AGENTS.md` (stamped from `templates/brand-routines/AGENTS.md`) that routes to `CLAUDE.md` and reads the output-style file, since Codex has no output-style layer. The creative skills' review gates gain an inline fallback for harnesses without subagent spawning. Known boundaries, by design: schedules stay Claude-cloud-only, hooks need a one-time interactive trust-and-approve per user (headless `codex exec` silently skips unapproved hooks), and the inline gates are weaker than fresh-context subagents. `sync-executable-layer.py` delivers `.codex/` and `AGENTS.md` on every pin bump; the symlink can't ride the sync, so real-step `migrations/v17.md` adds it to standing brains. |
