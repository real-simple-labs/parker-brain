...
- **NEW: Congruence — score the whole path, not one surface.** `.claude/skills/congruence/` joins one exact ad, its actual age-and-gender delivery and outcomes, and its rendered landing page. The canonical method is `creative-strategy-context/congruence.md`; the skill prefers Parker MCP, permits another authorized source or a complete direct packet, and blocks scoring when the creative, exact ad-level delivery, or destination experience is missing. Four seams roll into a fixed weighted score from 1 to 10, with the largest weighted loss driving the first repair. When a landing-page seam scores 6 or below, the skill studies the current brand and owned assets, then builds and visually verifies a local mockup while leaving the live site unchanged. Ships as `v16`; `migrations/v16.md` is a no-op because the normal executable-layer re-sync adds the new skill.
