...
- **The old-ads corpus lives in the knowledge tier.** In the active creative-strategy v1, the historical print-ad swipe library lives at `global/knowledge/creative-strategy/old-ads/` (README + INDEX + `entries/`, organized by industry and by mechanic), shipping into brand brains as `parker-system/creative-strategy-context/old-ads/` via onboarding and the propagate script's deliberate-add. The weekly idea hunt's historian lens retrieves from it; entries are describe-and-link, public-domain and openly archived material only.
