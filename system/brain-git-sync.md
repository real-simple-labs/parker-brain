...
3. **`commit-guard.py`** — Stop hook, the push-always enforcement. When a turn is about to end on a managed repo with uncommitted changes or commits that never got pushed, it blocks the stop once and tells the agent to run the save-and-push loop. Two loop-safety layers: `stop_hook_active` bounds it to one block per turn, and a fingerprint of the unsaved state (remembered in the OS temp dir) bounds it to one block per *state* — a persistent git error nobody can fix right now goes quiet instead of taxing every turn with a doomed retry, while any new work changes the fingerprint and re-arms the guard.
