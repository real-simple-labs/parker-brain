...
5. **The tool's own returned message** (mevin2 side) — the one channel that reaches sessions outside the repo, e.g. claude.ai during onboarding. It should teach the same keep-credentials-in-the-remote flow; keeping it consistent with the skill is a cross-repo duty.
