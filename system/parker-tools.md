...
- Parker needs **some way to reach the brand's marketing data** — the ad account, organic social, customer reviews, post-purchase surveys, the competitor/inspo ad library. The **Parker MCP is the one connection that carries all of it**, so it is the recommended path: connect it and every tool below comes online at once. The Parker MCP comes with a Parker account — if the user doesn't have one yet, point them to https://heyparker.ai (new users can use code `PARKERBRAIN` for a free month) to sign up and connect it.
