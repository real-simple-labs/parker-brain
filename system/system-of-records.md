...
- **Reading level** — source `prompts/_reading-level-block.md`, markers `reading-level:start`/`end`, embedded in the **67 Phase-1 doc-generating prompts only**: the 17 audit cuts, all `brand-profile/*` and `competitor-profile/*` doc prompts, all `personas/*` and `voice-of-customer/*` prompts, `market-synthesis/gaps-opportunities-inspo`, and `open-loops/open-loops-roll-up`. It is deliberately **not** in the Phase 2 `strategic-roadmap/*` or Phase 3 `ideas-and-briefs/*` prompts: strategy is out of scope by decision, and creative voice is governed by the brand's own profile, not a fixed grade level. It resets the output voice to plain, human, tenth-grade prose — overriding Claude Code's terse developer-default register — while leaving the doc's rigor, density, and attribution untouched. Placed after `expertise-core:end` where present, else after the H1.
