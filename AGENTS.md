...
- **Runtime tests:** `python3 -m unittest discover -s tests -v` runs the standard-library suite. CI checks Linux, macOS, and Windows. `python3 tests/probe_codex_runtime.py` additionally exercises an installed Codex CLI using localhost fixtures; no model inference or API key is needed. `evals/` remains a placeholder for marketing-output evaluations.
