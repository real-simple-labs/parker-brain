...
### 1. Brand dreaming — the five buckets

The core stream. Parker reads the day's comms for the brand and asks, across five buckets, what should change. Each bucket is a *proposal*, captured ungraded; the executing arm decides what actually lands.
...
Dreaming generates proposals; self-improvement governs whether they become canonical. Nothing dreaming proposes is applied on its own. All five buckets — context edits, skill changes, schedules, ideas, and open loops — route through the executing arm, and the human stays in the loop exactly as `self-improvement/the-living-loop.md` and `self-improvement/self-improvement-system.md` describe. Dreaming's reasoning is preserved whether a proposal is accepted or dismissed, so a rejected dream teaches Parker what does not land just as a rejected idea-bank idea or archived open loop does.
