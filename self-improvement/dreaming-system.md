...
## What dreaming reads — the whole connected picture

Dreaming reads **everything Parker can see**, and builds a complete, current picture of the user, the organization, the competitors, and the market. The Parker conversation is the richest single thread, but it is the center of a full sweep, not the whole of it. Every run pulls from every source wired in and reads it all against the vault.

- **The user and the organization — every connected connector.** The day's conversations with Parker first — pulled through Parker's own chat-history tool (`search_chat_history`), never assumed to be whatever session is running this pass. The user may have talked to Parker in the web app, so the day's comms live in Parker's history, not the local session; always pull them. Read what the user asked for, corrected, liked, rejected, wondered aloud about, kept coming back to — then every other connector they've linked: work email, Slack, calendar, task manager, Notion, docs, call and meeting transcripts, and the rest. Their real day and the org's real work live across those tools; the fuller the ecosystem dreaming reads each run, the truer it knows the person and the organization, and the more it can tee up. Read them all the same way, closely, for what is genuinely new.
- **The competitors and the landscape — go out into the world.** Reading the comms and the vault tells Parker what is already known; a dream also has to bring back what is *new from outside*. Actively go looking, don't just reread what's on file: pull the tracked competitor ad libraries and organic niche feeds fresh, run web searches on the category and the brand, and hunt for what moved since the last run — a competitor's new launch, ad, or pricing move, category press, a new entrant, a cultural moment or trend the brand could ride, a shift in what is winning on organic. Surface signals the vault does not yet contain. A dream that only repeats what is already on file did not dream, it summarized — every run should carry back at least something the brain did not already know.
- **The brand's accumulated knowledge is the ground, not the subject.** The brand's own context docs, open loops, hypotheses, validations, and idea bank are what the whole sweep is read *against* — to know whether something is genuinely new, contradicts what is on file, or closes something already open. The sweep is the figure; the vault is the ground.
- **Connect more, get smarter; name the blind spots.** A source not connected or not authorized this run is skipped and logged as a one-line blind spot ("can't see the team's Slack yet — connect it and I'll fold it in"), never guessed at. The more that is connected, the more complete the picture, and the reads say plainly what they could not see.
- **Sweep the work, not the private life.** The subject is the business, the organization, and the market — never the person's private world. Within any connected source, read only what bears on the work and skip anything that reads as personal or private: personal texts and DMs, a personal email thread, a family, health, relationship, or money matter, a calendar entry that is clearly personal. Purely personal channels — iMessage and the like — are not part of the sweep at all. When in doubt whether something is work or private, leave it out. A missed work signal is cheap; capturing someone's private life is a breach that cannot be taken back.
