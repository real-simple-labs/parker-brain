...
- **The user and the organization — every connected connector.** The day's conversations with Parker first — what the user asked for, corrected, liked, rejected, wondered aloud about, kept coming back to — then every other connector they've linked: email, Slack, calendar, task manager, Notion, docs, iMessage, call and meeting transcripts, and the rest. Their real day and the org's real work live across those tools; the fuller the ecosystem dreaming reads each run, the truer it knows the person and the organization, and the more it can tee up. Read them all the same way, closely, for what is genuinely new.
