...
# update-brain skills and their schedule recipes, and the static-ads craft skill with
# its static-ads.md doctrine doc). Add new ones there when the runtime ship list grows.
...
for repo in "${REPOS[@]}"; do
  ...
  if [ -d "$dir/parker-system" ]; then
    ...
    [ -d "$dir/.claude/skills/static-ads" ] || cp -R "$FACTORY/.claude/skills/static-ads" "$dir/.claude/skills/"
    cp -n "$FACTORY/global/knowledge/creative-strategy/static-ads.md" "$ps/creative-strategy-context/" 2>/dev/null || true
    ...
    cp -n "$FACTORY/templates/brand-routines/schedules/update-brain.md" "$dir/schedules/" 2>/dev/null || true
    [ -d "$dir/.claude/skills/static-ads" ] || cp -R "$FACTORY/.claude/skills/static-ads" "$dir/.claude/skills/"
    cp -n "$FACTORY/global/knowledge/creative-strategy/static-ads.md" "$dir/creative-strategy-context/" 2>/dev/null || true
  else
